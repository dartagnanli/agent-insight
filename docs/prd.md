...
- 每个项目目录对应独立的 SQLite 数据库
- 数据库路径：`{project_dir}/.agent-insight/insight.db`
- 通过 `cwd` 字段（Claude Code 传入）自动路由到对应数据库
- 全局仪表板可聚合展示多项目数据
...
Scenario: 项目级数据隔离
  ...
  Then /project-a/.agent-insight/insight.db 仅包含项目 A 的事件
  And /project-b/.agent-insight/insight.db 仅包含项目 B 的事件

Scenario: 全局视图
  When 用户执行 "agent-insight dashboard --global"
  Then 仪表板展示所有项目聚合数据
  And 可按项目切换过滤
```
...
storage:
  type: sqlite
  path: ""                          # 空=使用项目目录下 .agent-insight/insight.db
  retention_days: 30
