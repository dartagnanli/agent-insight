...
storage:
  ...
  path: ""                        # 数据库路径，空=项目目录下 .agent-insight/insight.db
...
```
<project_root>/.agent-insight/insight.db
```

可通过 `storage.path` 或 `AGENT_INSIGHT_DB_PATH` 自定义路径。目录权限 0700，数据库文件权限 0600。
...
每个项目使用独立的 `.agent-insight/insight.db`，通过 Claude Code 传入的 `cwd` 字段自动路由。切换项目目录后，数据自然隔离。
